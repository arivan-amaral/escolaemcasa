...
function pesquisa_relatorio_filtro($conexao,$texto,$escola){
   $sql = $conexao->query("SELECT $texto FROM aluno,ecidade_matricula,escola,turma WHERE ecidade_matricula.aluno_id = aluno.idaluno AND ecidade_matricula.turma_escola = escola.idescola AND ecidade_matricula.turma_id = turma.idturma AND ecidade_matricula.calendario_ano='2022' AND ecidade_matricula.matricula_ativa='S' AND ecidade_matricula.matricula_concluida='N'   AND ecidade_matricula.turma_escola=$escola ORDER BY escola.nome_escola ASC, turma.nome_turma ASC, aluno.nome asc;");
   return $sql->fetchAll();
}
