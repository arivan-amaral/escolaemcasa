...
function pesquisa_matricula_mensal($conexao,$escola,$ano){
   $sql = $conexao->query("SELECT * from ecidade_matricula, turma  where turma.idturma = turma_id and  matricula_situacao ='MATRICULADO' AND turma_escola='$escola' and calendario_ano = $ano  order by nome_turma asc");
   return $sql->fetchAll();
}

function pesquisa_relatorio_filtro($conexao,$texto,$sexo,$escola,$ano_letivo){
   $sql = $conexao->query("SELECT $texto FROM aluno,ecidade_matricula,escola,turma WHERE ecidade_matricula.aluno_id = aluno.idaluno AND ecidade_matricula.turma_escola = escola.idescola AND ecidade_matricula.turma_id = turma.idturma AND ecidade_matricula.calendario_ano='$ano_letivo' AND ecidade_matricula.matricula_ativa='S' AND ecidade_matricula.matricula_concluida='N' AND aluno.sexo = '$sexo'  AND ecidade_matricula.turma_escola=$escola ORDER BY escola.nome_escola ASC, turma.nome_turma ASC, aluno.nome asc;");
   
   return $sql->fetchAll();
}
function pesquisa_relatorio_filtro_todos($conexao,$texto,$escola,$ano_letivo){
   $sql = $conexao->query("SELECT $texto FROM aluno,ecidade_matricula,escola,turma WHERE ecidade_matricula.aluno_id = aluno.idaluno AND ecidade_matricula.turma_escola = escola.idescola AND ecidade_matricula.turma_id = turma.idturma AND ecidade_matricula.calendario_ano='$ano_letivo' AND ecidade_matricula.matricula_ativa='S' AND ecidade_matricula.matricula_concluida='N'   AND ecidade_matricula.turma_escola=$escola ORDER BY escola.nome_escola ASC, turma.nome_turma ASC, aluno.nome asc, aluno.bairro_endereco asc , aluno.endereco asc ;");
   
   return $sql->fetchAll();
}
function pesquisa_relatorio_filtro_quantidade_sexo($conexao,$escola,$ano_letivo,$idturm){
   $sql = $conexao->query("SELECT aluno.sexo, count(*) as 'quantidade' FROM aluno,ecidade_matricula,escola,turma WHERE ecidade_matricula.aluno_id = aluno.idaluno AND ecidade_matricula.turma_escola = escola.idescola AND ecidade_matricula.turma_id = turma.idturma AND ecidade_matricula.calendario_ano='$ano_letivo' AND ecidade_matricula.matricula_ativa='S'AND ecidade_matricula.matricula_ativa='S' and ecidade_matricula.turma_id='$idturm'  AND ecidade_matricula.matricula_concluida='N'   AND ecidade_matricula.turma_escola=$escola GROUP BY aluno.sexo ORDER BY  aluno.sexo ASC;");
