...
// Configurar os parâmetros da sessão para funcionar em modo privado
...
class LoadBalancer {
    ...
    public function getSubdomain() {
        // Tenta usar a sessão primeiro
        if (isset($_SESSION['assigned_subdomain']) && !empty($_SESSION['assigned_subdomain'])) {
        ...
        // Se no conseguir ler o arquivo, use round-robin simples
        if (!$connections) {
            $index = time() % count($this->subdomains);
            $selectedSubdomain = $this->subdomains[$index];
        } else {
            // Resto do código existente para encontrar o servidor com menos conexões
        ...
        // Garante que sempre teremos um subdomínio, mesmo que algo falhe
        if (!$selectedSubdomain) {
            $selectedSubdomain = $this->subdomains[0];
        }

        // Tenta salvar na sessão, mas não falha se não conseguir
        try {
            $_SESSION['assigned_subdomain'] = $selectedSubdomain;
        } catch (Exception $e) {
            error_log('Erro ao salvar na sessão: ' . $e->getMessage());
        }
...
// Verifica se  uma requisição de logout
