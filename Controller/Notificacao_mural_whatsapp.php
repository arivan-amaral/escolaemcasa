...
// *******************************************************************************************************************************
 ...
 if ($status_api) {//só ira atualizar no banco e enviar as mensagens se o status da api estives true
              ...
              $lista_mural="";
              $result_videos=$conexao->query("SELECT * FROM mural WHERE  serie_id=$serie_id AND setor='Secretaria'and notificado=0 and status='Ativo'  order BY data_hora ASC ");
              $cont_video=0;
              ...
              foreach ($result_videos as $key2 => $value2) {
                  ...
                  $lista_mural.="*TÍTULO:* $titulo\n*DESCRICÃO:* $descricao \n\n";                  
                  $conexao->exec("UPDATE mural set notificado=1 where id =$mural_id ");
...
// ********************************************************************************************************************
              ...
              $result_videos=$conexao->query("SELECT * FROM mural WHERE  serie_id=$serie_id and turma_id=$turma_id and escola_id=$escola_id and notificado=0 and status='Ativo'  order BY data_hora ASC ");
              ...
              if ($cont_video>0) {
                    ...
                    foreach ($result_aluno as $key4 => $value4) {
                        ...
                        $nome_aluno=$value4['nome'];
                         $mensagem_titulo="*________________MURAL________________*\n";
                         $mensagem=saudacao().", *".$nome_aluno."*, confira abaixo as últimas novidades que foram postadas no mural de sua turma:\n\n$lista_mural\nAcesse: https://escolaemcasalem.com.br/ e fique em dia com suas atividades 😉";
