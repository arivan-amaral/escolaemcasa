...
$url=str_replace($tirar, '',$url);
	...
	$conexao->exec("UPDATE  questao SET nome='$nome_descricao',pontos=$pontos where id=$idquestao ");
...
foreach ($_POST['idalternativa'] as $key => $value) {
	...
	$conexao->exec("UPDATE  alternativa SET nome='$alternativa' where id=$idalternativa ");
}
