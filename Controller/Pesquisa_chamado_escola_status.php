...
if ($pesquisa != '' || $data_inicial != '') {
    ...
    if($data_inicial == ''){
      if ($filtro == "ESCOLA" || $filtro == "TODOS") {
       ...
       if ($nome_escola == '') {
            $id_setor = 0;
            $res_nome_setor_2 = buscar_id_setor($conexao,$id_funcionario);
            foreach ($res_nome_setor_2 as $key => $value) {
              $id_setor = $value['setor_id'];
              $res_n_setor= buscar_setor_id($conexao,$id_setor);

                foreach ($res_n_setor as $key => $value) {
      ...
      if($filtro == "SOLICITANTE" ||  $filtro == "TODOS"){
    ...
    if($data_inicial != ''){
      if ($filtro == "ESCOLA" || $filtro == "TODOS") {
        // PESQUISA ESCOLA
          $res_escola = buscar_escola_por_nome($conexao,$pesquisa);
          foreach ($res_escola as $key => $value) {
            $id_escola_encontrado = $value['idescola'];
            $res =pesquisar_chamado_escola_data($conexao,$id_escola_encontrado,$data_inicial,$data_final);
            foreach ($res as $key => $value){
            $id_chamada = $value['id'];
            $status = $value['status'];
            $id_funcionario = $value['funcionario_id'];
      ...
      if($filtro == "SOLICITANTE" ||  $filtro == "TODOS"){
         $id_funcionario_encontrado = 0;
          $res_funcionario = id_funcionario($conexao,$pesquisa);
        foreach ($res_funcionario as $key => $value) {
          $id_funcionario_encontrado = $value['idfuncionario'];
        }  
        $res=pesquisar_chamado_solicitante_data($conexao,$id_funcionario_encontrado,$data_inicial,$data_final);
          foreach ($res as $key => $value){
            $id_chamada = $value['id'];
            $status = $value['status'];
            $id_funcionario = $value['funcionario_id'];
      ...
      if($filtro == "SETOR" ||  $filtro == "TODOS"){
        ...
        foreach ($res_setor as $key => $value) {
          $id_setor = $value['id'];
          $res=pesquisar_chamado_setor_data($conexao,$id_setor,$data_inicial,$data_final);
          foreach ($res as $key => $value){
          $id_chamada = $value['id'];
          $status = $value['status'];
          $id_funcionario = $value['funcionario_id'];
