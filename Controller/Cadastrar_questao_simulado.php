		...
		if ( strstr ( '.pdf;.docx;.doc;.txt;.odt;.pptx;.jpg;.jpeg;.gif;.png;.PNG', $extensao ) ) {	    	    
			...
			if(move_uploaded_file($arquivo_tmp, $destino)){
				...
				foreach ($_POST['escola_turma_disciplina'] as $key => $value) {
					...
					$result=cadastrar_questao_simulado($conexao,$nome_descricao, $tipo, $pontos,$questionario_id,$origem_questionario_id);

					$questao_id=$result[1];
					$resultado_arquivo=cadastrar_arquivo_simulado($conexao,$novoNome, $questao_id, $extensao,$origem_questionario_id);
...
foreach ($array_idquestao as $key => $value) {
	...
	if ($tipo=="multipla" || $tipo=="multipla_justificada"){
		for ($i=1; $i < 6 ; $i++) {
			...
			cadastrar_alternativa_simulado($conexao, $alternativa, $tipo, $questao_id,$origem_questionario_id);
		}

	}else{
		cadastrar_alternativa_simulado($conexao,$nome, $tipo,$questao_id,$origem_questionario_id);
	}
